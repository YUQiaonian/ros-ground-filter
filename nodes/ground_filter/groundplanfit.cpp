...
GroundPlaneFit::GroundPlaneFit():node_handle_("~"){
    ...
    node_handle_.param<std::string>("point_topic", point_topic_, "/velodyne_cloud_registered");
...
void GroundPlaneFit::velodyne_callback_(const sensor_msgs::PointCloud2ConstPtr& in_cloud_msg){
    ...
    for(int i=0;i<num_iter_;i++){
     ...
     for(int i=0;i<g_not_ground_pc->points.size();i++){
        g_not_ground_pc->points[i].z=0;
     }
